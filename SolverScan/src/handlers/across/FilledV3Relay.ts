...
export const FilledV3RelayHandler = async (
  ...
  if(!bridgedata){
    await bridgeDataDB.create({
      ...
      solver: relayer,
      solverGasCost: relayExecutionInfo.gasUsed.toString(),
    ...
    bridgedata.solverGasCost = relayExecutionInfo.gasUsed.toString();
  ...
  if(!solverdata){
    await solveranalysisDB.create({
      ...
      totalGasSpent: relayExecutionInfo.gasUsed.toString()
    });
  }
  else{
    ...
    solverdata.totalGasSpent = (parseInt(solverdata.totalGasSpent) + parseInt(relayExecutionInfo.gasUsed)).toString();
